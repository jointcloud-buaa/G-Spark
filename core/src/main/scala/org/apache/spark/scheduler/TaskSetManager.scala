...
private[spark] class TaskSetManager(
  ...
  private val conf = sched.ssc.conf
  ...
  val epoch = sched.mapOutputTracker.asInstanceOf[MapOutputTrackerMasterRole].getEpoch
    ...
    if (!isZombie && !offerBlacklisted) {
     ...
     dequeueTask(execId, host, allowedLocality).map { case ((index, taskLocality, speculative)) =>
        ...
        val info = new TaskInfo(taskId, index, attemptNum, curTime,
          execId, host, taskLocality, speculative, task.isFakeTask)
        ...
        sched.stageScheduler.taskStarted(task, info)
   ...
   */
  // TODO-lzp: 此处改变了方法的可见域, 看能不能改回来
  private[spark] def abortIfCompletelyBlacklisted(
  ...
  def handleTaskGettingResult(tid: Long): Unit = {
    ...
    sched.stageScheduler.taskGettingResult(info)
  }
  ...
  def handleSuccessfulTask(tid: Long, result: DirectTaskResult[_]): Unit = {
    ...
    sched.stageScheduler.taskEnded(tasks(index), Success, result.value(), result.accumUpdates, info)
  ...
  def handleFailedTask(tid: Long, state: TaskState, reason: TaskFailedReason) {
    ...
    sched.stageScheduler.taskEnded(tasks(index), reason, null, accumUpdates, info)
  ...
  def abort(message: String, exception: Option[Throwable] = None): Unit = sched.synchronized {
    ...
    sched.stageScheduler.taskSetFailed(taskSet, message, exception)
    isZombie = true
    maybeFinishTaskSet()
  }
   ...
   if (tasks(0).isInstanceOf[ShuffleMapTask] && !env.blockManager.externalShuffleServiceEnabled) {
      for ((tid, info) <- taskInfos if info.executorId == execId) {
        ...
        if (successful(index)) {
          ...
          sched.stageScheduler.taskEnded(
